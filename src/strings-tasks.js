...
function repeatString(str, times) {
  return times > 0 ? str.repeat(times).trim() : '';
}
...
function sumOfCodes(str) {
  return str ? str.split('').reduce((acc, ch) => acc + ch.charCodeAt(), 0) : 0;
}
 ...
 *
 * @example
 *   invertCase('Hello, World!') =>  'hELLO, wORLD!'char === char.toLowerCase() ? char.toUpperCase() : char.toLowerCase()
...
function encodeToRot13(str) {
  return str.replace(/[A-Za-z]/g, (char) => {
    ...
    const base = char === char.toUpperCase() ? upperBase : lowerBase;
...
function getCardId(value) {
  ...
  const sym = value.at(-1);
  const num = value.slice(0, -1);
  return numArr.indexOf(num) + symbArr.indexOf(sym) * base;
}
